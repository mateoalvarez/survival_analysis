...
\section{Technologies}
  ...
  \subsection{Apache Spark}
    ...
    \subsubsection{Apache Spark Structure}
      ...
      Spark driver program has to be in continuous contact with the executors, as the results calculated by them will return to the driver at the end. The driver can run in an external machine of the cluster, but, as it has to be in continuous communication with the executor programs, it has to be running all the time the executors are calculating, it can not be disconnected to the cluster and it is important to have a good connection in order not to lose performance.
  ...
  \subsection{Apache SystemML}
    ...
    \subsubsection{SystemML Architecture}
      ...
      \subsubsection*{Hight-level Operators (HOPs)}
        ...
        This phase of the optimization consists on structuring the operations in order to be able to rewrite the code in a more optimized way, for example changing the order of operations when it implies less resource consumption.
