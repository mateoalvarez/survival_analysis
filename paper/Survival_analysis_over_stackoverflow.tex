    ...
    \subsubsection{Apache Spark Data Structure}
      ...
      \subsubsection{Spark workflow}
        ...
        \subsubsection*{Scheduling and executing the physical plan on the cluster}
          ...
          Finally, the stages are executed in order, launching the tasks over the available nodes to compute the resulting RDD. As the execution runs in-memmory, whenever a task fails, the entire sequence of operations of the stage has to be computed for the particular lost partition, but not all the tasks for all partitions. For this reason it is common to \emph{cache} the RDDs after a series of operations, avoiding this way to reacalculate all the previous steps.

      \subsubsection{Spark UI}

        All the information regarding the running application is
